...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TKStug Whiteboard Animation Studio - Dựng Video Tranh Phác Thảo & Tô Màu Nước AI (Mô Hình 1)")
    ...
    def setup_ui(self):
        ...
        title_label = QLabel("🎨 TKStug Whiteboard Animation Studio (Mô Hình 1)")
        title_label.setStyleSheet("font-size: 17px; font-weight: 900; color: #38bdf8;")
        left_layout.addWidget(title_label)

        sub_label = QLabel("Mô Hình Phác Thảo Nghệ Thuật, Bàn Tay Quét Màu Nước & Camera 2.5D Zoom")
        ...
        btn_row = QHBoxLayout()
        self.btn_auto_segment = QPushButton("⚡ Tự Động Phân Cảnh & Khớp Tranh")
        ...
        preview_group = QGroupBox("3. Màn Hình Xem Trước Nét Chì & Quét Màu Nước (Live Player)")
        ...
        prev_ctrl.addWidget(QLabel("Tranh minh họa:"))
        ...
        self.chk_camera = QCheckBox("Hiệu ứng Camera Pan & Zoom điện ảnh")
        ...
        self.btn_export = QPushButton("🚀 Xuất Toàn Bộ Video MP4 Nghệ Thuật Hoàn Chỉnh (Full HD)")
...
Khi bạn đặt ra mục tiêu rõ ràng và tập trung cao độ, bạn sẽ tìm thấy chiếc chìa khóa mở ra cánh cửa thành công và bước lên đỉnh vinh quang."""
    ...
    def on_auto_segment(self):
        ...
        if self.current_scenes:
            ...
            self.lbl_status.setText(f"Đã phân chia thành {len(self.current_scenes)} phân cảnh nghệ thuật tự động!")
    ...
    def on_upload_custom_image(self):
        ...
        if file_path:
            ...
            self.canvas_widget.set_scene(self.current_scenes[row], self.cb_theme.currentData())
            self.lbl_status.setText(f"Đã nạp ảnh riêng cho Cảnh {row+1}: {os.path.basename(file_path)}")
    ...
    def on_start_export(self):
        ...
        save_path, _ = QFileDialog.getSaveFileName(
            self,
            "Lưu File Video MP4 Nghệ Thuật",
            os.path.join(os.path.expanduser("~"), "Desktop", "Video_Artistic_Story.mp4"),
            "Video Files (*.mp4)"
        )
    ...
    def on_render_finished(self, success: bool, msg: str):
        ...
        if success:
            ...
            self.lbl_status.setText("Xuất Video Nghệ Thuật thành công 100%! 🎉")
            ret = QMessageBox.information(
                self,
                "Hoàn Thành Xuất Video!",
                f"Video Nghệ Thuật của bạn đã được xuất thành công tại:\n\n{msg}\n\nBạn có muốn mở thư mục chứa video không?",
